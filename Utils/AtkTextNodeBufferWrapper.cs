...
namespace Echoglossian.Utils
{
    ...
    internal unsafe class AtkTextNodeBufferWrapper
    {
        ...
        public void SetBuffer(string text)
        {
            ...
            if (strBuffer == null || strLen + 1 > bufferLen)
            {
                NativeMemory.Free(strBuffer);
                strBuffer = (byte*)NativeMemory.Alloc((nuint)(strLen + 1)); // Need one extra byte for the null terminator
                bufferLen = strLen + 1;
            }

            // Wrap buffer in a span to use GetBytes
            Span<byte> bufferSpan = new(strBuffer, strLen + 1);
